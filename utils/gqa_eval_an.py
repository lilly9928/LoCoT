...
parser.add_argument('--tier',           default = "testdev",                     type = str,    help = "Tier, e.g. train, val")
...
questions = loadFile(args.questions.format(tier = args.tier),path='/data2/NS/GQA/questions')

# # Load choices
# print("Loading choices...")
# choices = loadFile(args.choices.format(tier = args.tier))

# Load predictions and turn them into a dictionary/data3/KJE/modelLog/Logic_LLama/gqa_llava_mistral7b_0724_all/testdev_predictions.json
print("Loading predictions...")
predictions = loadFile(args.predictions.format(tier = args.tier),path='/data3/KJE/modelLog/Logic_LLama/GQA/gqa_llava_mistral7b_0724_all/')
predictions = {p["questionID"][0]: p["prediction"] for p in predictions}
...
for qid, question in tqdm(questions.items()):
    ...
    if question["isBalanced"]:
        ...
        if score ==0.0:
            if step[str(stepsNum)]: 
                step[str(stepsNum)].append({
                    'qid':qid,
                    'gold':gold,
                    'predicted':predicted
                })
