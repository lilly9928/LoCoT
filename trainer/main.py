...
def main():
    ...
    if opt.data =='aokvqa':
        ...
        dataset = data.SCIENCEQA_Dataset(
            data=train_examples['test'], #split파일
            image_dir='/data3/KJE/vqa/scienceqa/test/',
            split = 'test'
        )
    ...
    if opt.data=='scienceqa':
        PROMPT_CONCLUSION_GENERATION, PROMPT_OBJECT_SELECTION, \
        PROMPT_VLM_PREMISES, PROMPT_LLM_PREMISES,PROMPT_SCORING, PROMPT_INFERENCE = prompt.Prompt4LM(opt.lvlm_name, opt.num_object_premises,opt.num_llm_premises,opt.cot,'choice')
    ...
    for batch in tqdm(dataloader):
        ...
        if opt.data == 'aokvqa':
            ...
            question_id,images,image_path,question,target,value,choices= batch
        ...
        if 'score_only' not in opt.data:
            ...
            if image_path[0]!= 'no': 
                ...
                c2_p = PROMPT_OBJECT_SELECTION.format(object=' ', question=question[0])
        ...
        if opt.inference_model =='vlm':
            ...
            if opt.data=='scienceqa':
                selection = ['A.','B.','C.','D.','E.']
                for i in range(len(answer_candidate)):
                    choice_string=f"{selection[i]} {answer_candidate[i]}\n"
                c3 = PROMPT_INFERENCE.format(question=question[0], premise=str(new_premise),options=choice_string)
            else:
            ...
            answer_index = selection.index(answer+'.')
        ...
        if opt.debug:
            ...
            print(f"GT: {selection[target[0]].split('.')[0]}\n")
            print(f"answer: {answer}\n")
            print(f"answer_index: {answer_index}\n")


        if target[0]!= None:
            if opt.data == 'gqa' or opt.data == 'tdiuc' or opt.data =='gqa_case'or opt.data =='scienceqa':
                if answer.lower().strip() == selection[target[0]].split('.')[0].lower().strip():
                    score = 1
                else:
                    score = 0
            else:
    ...
    name_path = os.path.join(opt.logger_path, opt.name)    
    if not os.path.exist(name_path):
        os.makedirs(name_path)
    
    all_predict_data_path = os.path.join(opt.logger_path,opt.name, f'all_predictions_{opt.data}_{opt.split}.json')
    temp_save_path = os.path.join(opt.logger_path,opt.name, f'temp_results_rank_{local_rank}.json')

    predict_data_path = os.path.join(opt.logger_path, opt.name, f'predictions_{opt.data}_{opt.split}.json')
    ...
    if opt.data == 'aokvqa' or  opt.data == 'okvqa' or opt.data == 'gqa':
