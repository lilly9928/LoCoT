...
def extract_question_answer_rationale(text):
    question_match = re.search(r'question:\s*(.*)\s*Respond', text)
    answer_match = re.search(r'Answer:\s*(.*)\s*,\s*Logic Rule:', text)
    rationale_match = re.search(r'Logic Rule:\s*(.*)\s*$', text)
