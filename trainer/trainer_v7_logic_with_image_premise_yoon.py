...
from transformers import AutoModelForCausalLM,AutoTokenizer,BitsAndBytesConfig,LlavaNextProcessor, LlavaNextForConditionalGeneration,Blip2Processor,Blip2ForConditionalGeneration, CLIPProcessor, CLIPModel, BlipProcessor, BlipForConditionalGeneration, ViltProcessor, ViltForQuestionAnswering, FlavaProcessor, FlavaModel,Blip2Processor, Blip2ForConditionalGeneration, InstructBlipProcessor, InstructBlipForConditionalGeneration
...
def main():
    ...
    if opt.data =='aokvqa':
        ...
        train_examples = GQAdata.loadFile(
            '/data5/yoon/error_case_data.json'
            # '/data2/NS/GQA/100_split_testdev_data.json'
        )
        dataset = GQAdata.trainerV3_Dataset(
            data=train_examples['questions'],
    ...
    elif opt.lvlm_name == 'Salesforce/blip2-flan-t5-xl':
        processor = Blip2Processor.from_pretrained(opt.lvlm_name, cache_dir=opt.cache_dir)
        vlm_model = Blip2ForConditionalGeneration.from_pretrained(opt.lvlm_name,
                                                                      torch_dtype=torch.float16,
                                                                        low_cpu_mem_usage=True,
                                                                        cache_dir=opt.cache_dir,
                                                                        )
    elif opt.lvlm_name == 'Salesforce/instructblip-flan-t5-xl':
        processor = InstructBlipProcessor.from_pretrained(opt.lvlm_name, cache_dir=opt.cache_dir)
        vlm_model = InstructBlipForConditionalGeneration.from_pretrained(opt.lvlm_name,
                                                                      torch_dtype=torch.float16,
                                                                        low_cpu_mem_usage=True,
                                                                        cache_dir=opt.cache_dir,
                                                                        )
    ...
    if opt.lvlm_name != 'llava-hf/llava-v1.6-vicuna-13b-hf':
        ...
        #vlm_model = vlm_model.to_empty(device=local_rank)
    ...
    for batch in tqdm(dataloader):
        ...
        if 'score_only' not in opt.data:
            ...
            for c2 in object_candidate:
                ...
                for answer in answer_candidate:
                    ...
                    vlm_inputs = processor( p1_p,crop_image, return_tensors="pt", max_length=512)
                    vlm_inputs = {k: v.to(local_rank) for k, v in vlm_inputs.items()}
        ...
        for ans in answer_candidate:
            ...
            for premise in premise_for_answer[ans]:
                ...
                if opt.scoring_LLM:
                    ...
                    save_score.append(f"{premise} ({score})")  # 프레미스와 스코어를 함께 저장
                    ...
                    if score > opt.threshold:
                        new_premise[ans].append(f"{premise} ({score})")  # 프레미스와 스코어를 함께 저장
        ...
        if opt.data == 'aokvqa':
            ...
            all_predict_data.append({
                ...
                "group": group,
                "final_answer": answer,
                "premise_score": save_score  # 프레미스와 스코어가 결합된 데이터
            })      
        elif opt.data == 'tdiuc':
            all_predict_data.append({
                "imagepath": image_path,
