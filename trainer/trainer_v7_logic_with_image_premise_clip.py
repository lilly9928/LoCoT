...
def main():
    ...
    for batch in tqdm(dataloader):
        ...
        for ans in answer_candidate:
            ...
            for premise in premise_for_answer[ans]:
                if opt.scoring_LLM:
                    ...
                    probs = logits_per_image.softmax(dim=1).cpu().detach().numpy()[0][0]
                    # print('==============',probs)
                    answer_score_dic[ans] += probs
                    save_score.append(str(probs))
                    score = probs

                    # if score > 0.5:
                    local_logic_scores.append(score)
                    # print("============ CLIP INPUTS", clip_inputs)
                ...
                # save_score.append(premise+str(score))
                    ...
                    if score > opt.threshold:
                        new_premise[ans].append(premise)
            logic_scores.append(np.array(local_logic_scores))
            
        ######################### INFERENCE (FUZZY LOGIC) ###################################
        print('Fuzzy logic calculation v2')
        print('clip score: ', logic_scores)
        threshold = .3
        logic_scores = np.array(logic_scores, dtype=object)
        # filter by threshold and check unique element before min
        add_threshold = True
        while True:
            while True:
                filtered_logic_scores = []
                not_zero = []
                print('=====')
                print('th', threshold)
                for logic_score in logic_scores:
                    filtered_logic_score = logic_score[np.where(logic_score > threshold)[0]]
                    print(filtered_logic_score)
                    filtered_logic_scores.append(filtered_logic_score)
                    not_zero.append(filtered_logic_score.shape[0])
                print(filtered_logic_scores)
            ...
            print('all logic score', logic_scores)
            print('filtered logic score', filtered_logic_scores)
            filtered_logic_scores = np.array(filtered_logic_scores, dtype=object)
            min_values = []
            for row in filtered_logic_scores:
                try:
                    min_values.append(np.min(row))
                except:
                    min_values.append(0)
            min_values = np.array(min_values)
            unique_elts = np.unique(min_values)
            print("unique element", unique_elts)
            # threshold -= 0.025
            print('num unique elts', len(unique_elts))
            # dynamic threshold v2 (uncommend to apply and put the 'break' inside 'else')
            # if len(unique_elts) == 1 and add_threshold:
            #     threshold += 0.5
            #     add_threshold = False
            # else:
            break

            # if len(unique_elts) > 1:
        # break
            
        # try:
        #     min_values = np.array([np.min(row) for row in logic_scores]) # 'or' operator
        # except:
        #     min_values = 0
        max_of_min = np.max(min_values) # 'and' operator
        max_of_min_loc = np.where(min_values == max_of_min)[0][0]
        final_answer = answer_candidate[max_of_min_loc]
        print('Selected answer with fuzzy logic (min, max) = ', min_values, final_answer)
        ...
        all_predict_data[-1]['final_answer_fuzzy'] = final_answer
