...
if __name__ == "__main__":
    ...
    logging.basicConfig(filename='/data3/KJE/modelLog/Logic_LLama/run_trainer_v4.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
    ...
    vlm_model = Blip2ForConditionalGeneration.from_pretrained(vlm_name, device_map='auto', cache_dir=cache_dir)
    ...
    for batch in tqdm(dataloader):
        ...
        if proNouns: # pronoun 있으면 pronoun 으로 가장 관련있는 단어 리턴 (오브젝트 리스트 중에 선택) : llm 예외 존재
            ...
            if pred_pronouns in image_object: # 오브젝트 리스트에서 추출했을 경우
                ...
                vlm_inputs = processor(crop_image, prompt.concept_caption_prompt(question,pred_pronouns,idx_pre_object), return_tensors="pt").to( "cuda", torch.float16)
                # vlm_inputs = {k: v.to(local_rank) for k, v in vlm_inputs.items()}
            else:
                idx_pre_object = None #전체 이미지에서 캡션 생성
                vlm_inputs = processor(images, prompt.concept_caption_prompt(question,pred_pronouns,idx_pre_object), return_tensors="pt").to("cuda", torch.float16)
                # vlm_inputs = {k: v.to(local_rank) for k, v in vlm_inputs.items()}
            ...
            if debug:
                logging.debug('proNouns: ',proNouns)
                logging.debug('Predict ProNouns: ',pred_pronouns)
                logging.debug('Predict ProNouns Caption: ',pred_pronouns_caption)
        ...
        if debug:
            logging.debug('pred_object in image_object: ', pred_object in image_object)
            logging.debug('pred_object: ', pred_object)
            logging.debug('Predict object Caption: ', pred_object_caption)
        ...
        if debug:
            logging.debug('logic_result ', logic_result)
            logging.debug('logic_result_clean ', logic_result_clean)
        ...
        if debug:
            logging.debug('final prompt ', fprompt)
        ...
        answer_rationle_data_path = os.path.join(save_path,f'without_iter_results_okvqa_test.json')
        all_predict_data_path = os.path.join(save_path,f'without_iter_all_predict_data_okvqa_test.json')
