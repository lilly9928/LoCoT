...
def main():
    ...
    dataloader = DataLoader(dataset, batch_size=opt.per_gpu_batch_size, shuffle=False,sampler=sampler,
    ...
    llava = llava.to(rank)
    llava = DDP(llava, device_ids=[rank])
    ...
    for batch in tqdm(dataloader):
        ...
        if answer_model == 'blip':
            ...
            vlm_inputs = processor(images, question[0], return_tensors="pt").to(rank)
            ...
            inputs = llava_processor(answer_prompt, images[0], return_tensors="pt").to(rank)
            output = llava.module.generate(**inputs, max_new_tokens=100)
        ...
        for pre_answer in answer_list:
            commonsense_premise_prompt = prompt.LLM_PROMPT_FORMAT_COMMONSENSE_PREMISE.format(question=question[0],
                                                                                             answer=pre_answer)
            input_tokens = llm_tokenizer(commonsense_premise_prompt, return_tensors="pt")["input_ids"].to(rank)
        ...
        c2 = prompt.VLM_PROMPT_FORMAT_FINAL_ANSWER.format(question=question[0], answer=pre_answer,
                                                          information=str(global_premise) + ' ' + str(object_premise),
                                                          object_name=pre_answer, cs=str(cs_dir))

        inputs = llava_processor(c2, images[0], return_tensors="pt").to(rank)
        output = llava.module.generate(**inputs, max_new_tokens=100)
        output = llava_processor.decode(output[0], skip_special_tokens=True)
        ...
        answer_rationle_data_path = os.path.join(opt.logger_path, opt.name,
                                                 f'new_version_blip2_Mistral_Llava_results_okvqa_test_0703.json')
        all_predict_data_path = os.path.join(opt.logger_path, opt.name,
                                             f'new_version_blip2_Mistral_Llava_all_predict_data_okvqa_test_0703.json')
