...
def main():
    ...
    for batch in tqdm(dataloader):
        ...
        outputs = vlm_model.module.generate(**vlm_inputs, max_length=300,
                                     num_beams=5,
        ...
        for ans in answer_candidate:
            ...
            p1_p = prompt.LLAVA_PROMPT_FORMAT_ANSWER_PREMISE_ver2.format(question=question[0], answer=ans)
        ...
        ans_premises = []
        for ans in answer_candidate:
            p2 = prompt.LLM_PROMPT_RULE_GENERATION.format(question=question, answer=ans)
            ...
            for premise in generated_text:
                ...
                out = util.extract_answer_candidate(output)
                answer_score_dic[ans] += util.get_score(out)
                answer = max(answer_score_dic, key=answer_score_dic.get)

        # print(answer_score_dic)
        # print(ans_premises)
